      ...
      if(closest.m_type==Collision::Type::kHit){//if collision, get the color of the object it collided with
        ...
        for(int l=0;l<_scene.getLights().size();l++){
          ...
          if(dist2+.01>dist1){
            ...
            collisionColor.x+=angleDimming*sceneLight.getD().x*obstruction.m_material->kD.x;
            collisionColor.y+=angleDimming*sceneLight.getD().y*obstruction.m_material->kD.y;
            collisionColor.z+=angleDimming*sceneLight.getD().z*obstruction.m_material->kD.z;
          }
        }
